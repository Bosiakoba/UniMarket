...
class SellerStore extends ChangeNotifier {
  ...
  double get sellerRating {
    final withReviews =
        _records.where((r) => r.listing.reviewCount > 0).toList();
    if (withReviews.isEmpty) return 0.0;
    final totalReviews =
        withReviews.fold(0, (sum, r) => sum + r.listing.reviewCount);
    final weightedSum = withReviews.fold<double>(
      0,
  ...
  int get totalRatingCount {
    final withReviews =
        _records.where((r) => r.listing.reviewCount > 0).toList();
    return withReviews.fold(0, (sum, r) => sum + r.listing.reviewCount);
  }
  ...
  bool get meetsFollowersCriteria =>
      followersCount >= VerificationCriteria.minFollowers;

  bool get meetsRatingCriteria =>
      sellerRating >= VerificationCriteria.minSellerRating &&
      totalRatingCount >= VerificationCriteria.minRatingCount;
  ...
  ListingItem _listingFromDraft(
    ...
    return ListingItem(
      ...
      location: preserveFrom?.location ?? sellerUniversity,
      category: draft.category,
  ...
  Future<String?> submitAppeal({
    ...
    if (client != null && isLiveSession(client)) {
      try {
        await client.submitListingAppeal(listingId, reason);
      } catch (error) {
        return error.toString();
      }
    }
    ...
    if (index != -1) {
      _records[index] = _records[index].copyWithListing(
        _records[index].listing.copyWith(appealComment: reason),
      );
      notifyListeners();
    }
    return null;
  ...
  Future<void> syncFromApi(ApiClient client, {required AppUser user}) async {
    if (isLiveSession(client)) {
      try {
        final cachedListings = client.getCached('/api/listings?q=&sort=verified');
        final cachedMyListings = client.getCached('/api/listings/my');
        if (cachedListings != null) {
          final List<dynamic> list = jsonDecode(cachedListings);
          _remoteCatalog = list.map((item) => ListingMapper.fromJson(item as Map<String, dynamic>)).toList();
          useRemoteCatalog = true;
        }
        if (cachedMyListings != null) {
          final List<dynamic> myList = jsonDecode(cachedMyListings);
          _records.clear();
          for (final item in myList) {
            _upsertOwnedListing(ListingMapper.fromJson(item as Map<String, dynamic>));
          }
        }
        if (cachedListings != null || cachedMyListings != null) {
          notifyListeners();
        }
      } catch (_) {}
    }
    ...
    try {
      ...
      if (isLiveSession(client)) {
        final myItems = await client.fetchMyListings();
      ...
      if (!isLiveSession(client)) {
        ...
        // Keep cached items so products do not vanish in low connectivity.
        useRemoteCatalog = true;
      }
    } finally {
      isSyncingCatalog = false;
