...
class NotificationStore extends ChangeNotifier {
  ...
  int get unreadCount => _items.where((n) => !n.isRead && n.type != NotificationType.message).length;

  Future<void> syncFromApi(ApiClient client) async {
    if (!isLiveSession(client)) return;

    final cached = client.getCached('/api/notifications');
    if (cached != null) {
      try {
        final List<dynamic> list = jsonDecode(cached);
        final cachedItems = list.map((item) => ListingMapper.notificationFromJson(item as Map<String, dynamic>)).toList();
        _items
          ..clear()
          ..addAll(cachedItems);
        notifyListeners();
