...
class ListingGridScreen extends StatelessWidget {
  ...
  List<ListingItem> _listings(List<ListingItem> catalog, {AppUser? user}) {
    return switch (mode) {
      ListingGridMode.feedSection =>
        HomeFeedService.allListingsForSection(sectionId!, catalog, user: user),
    ...
    return ListenableBuilder(
      ...
      builder: (context, _) {
        ...
        final section = mode == ListingGridMode.feedSection
            ? HomeFeedService.sectionForId(sectionId!, catalog, user: user)
            : null;
        final listings = _listings(catalog, user: user);
