...
public static class DatabaseSchemaPatcher
{
    public static async Task ApplyAsync(AppDbContext db, CancellationToken ct = default)
    {
        ...
        await EnsureColumnAsync(
            ...
            table: "Messages",
            column: "EditedAt",
            alterSql: "ALTER TABLE Messages ADD COLUMN EditedAt TEXT NULL;",
            ct);
        await EnsureColumnAsync(
            db,
            table: "Messages",
            column: "DeletedAt",
            alterSql: "ALTER TABLE Messages ADD COLUMN DeletedAt TEXT NULL;",
            ct);
        await EnsureColumnAsync(
            db,
            table: "Messages",
            column: "DeletedForSenderId",
            alterSql: "ALTER TABLE Messages ADD COLUMN DeletedForSenderId TEXT NULL;",
            ct);

        await db.Database.ExecuteSqlRawAsync(
            """
            CREATE TABLE IF NOT EXISTS CarouselBanners (
                Id TEXT NOT NULL CONSTRAINT PK_CarouselBanners PRIMARY KEY,
                Title TEXT NOT NULL,
        ...
        CancellationToken ct)
    {
        var columns = table switch
        {
            ...
            "Messages" => await db.Database
                .SqlQueryRaw<ColumnInfo>("PRAGMA table_info(Messages)")
                .ToListAsync(ct),
