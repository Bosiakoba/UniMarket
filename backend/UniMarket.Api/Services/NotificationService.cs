...
public class NotificationService(AppDbContext db, FirebaseNotificationService fcm)
{
    public async Task<UserNotification> CreateAsync(
        ...
        IReadOnlyDictionary<string, string>? extraFcmData = null)
    {
        ...
        if (extraFcmData is not null)
        {
            foreach (var (key, value) in extraFcmData)
            {
                fcmData[key] = value;
            }
        }

        await fcm.SendAsync(tokens, title, body, fcmData, ct);
