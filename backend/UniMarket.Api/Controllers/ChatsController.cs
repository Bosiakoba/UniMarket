...
public class ChatsController(
    ...
    UnreadMessageEmailDispatcher emailDispatcher,
    IHubContext<ChatHub> chatHub,
    FirebaseNotificationService fcm) : ControllerBase
{
    ...
    public async Task<ActionResult<IEnumerable<MessageDto>>> Messages(string chatId, CancellationToken ct)
    {
        ...
        {
            if (message.DeletedForSenderId == currentUser.UserId)
                continue;
        ...
        CancellationToken ct)
    {
        ...
        var senderName = sender?.FullName ?? "UniMarket";
        ...
        await chatHub.Clients
            ...
            .SendAsync("NewMessage", signalRPayload, ct);

        // ---- 2. Push notification (FCM) — data-only, no DB notification record ----
        ...
        emailDispatcher.EnqueueDelayedEmail(chat.Id, message.Id, recipientId);

        return Ok(new { id = message.Id, sentAt = message.SentAt });
    }
        ...
        CancellationToken ct)
    {
        ...
        if (message.SenderId != currentUser.UserId) return Forbid();
        if (message.MessageType != "text") return BadRequest(new { message = "Only text messages can be edited." });
        if (message.DeletedForSenderId is not null) return BadRequest(new { message = "Cannot edit a deleted message." });
        ...
        return Ok(new { editedAt = message.EditedAt });
    }
        ...
        CancellationToken ct = default)
    {
        ...
        if (forEveryone)
        {
            ...
            {
                return BadRequest(new { message = "You can only delete messages for everyone within 1 hour of sending." });
            }

            message.DeletedForSenderId = "*";
        ...
        {
            message.DeletedForSenderId = currentUser.UserId;
            message.DeletedAt = DateTime.UtcNow;
            await db.SaveChangesAsync(ct);
        }
    ...
    private async Task<ChatDto> ToDto(Chat chat, CancellationToken ct)
    {
        ...
        // Resolve online presence from the in-memory presence store.
        var presenceService = HttpContext.RequestServices.GetRequiredService<UserPresenceStore>();
        var (isOnline, lastSeenAt) = presenceService.GetPresence(otherUserId);
        ...
        return new ChatDto(
            ...
            OtherPartyIsOnline: isOnline,
            OtherPartyLastSeenAt: lastSeenAt);
    }
