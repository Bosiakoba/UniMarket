    ...
    R2StorageService storage) : ControllerBase
{
        ...
        CancellationToken ct)
    {
        ...
        var listing = new Listing
        {
            ...
            MeetupLocation = !string.IsNullOrWhiteSpace(user.Campus) && user.Campus.ToLower() != "none" && user.Campus != "Main Campus"
        ...
        CancellationToken ct)
    {
        ...
        var newUrls = new HashSet<string>(
            request.PhotoUrls.Select(url => storage.NormalizeMediaUrl(url)),
            StringComparer.OrdinalIgnoreCase);

        foreach (var oldImage in listing.Images)
        {
            var oldUrl = storage.NormalizeMediaUrl(oldImage.ImageUrl);
            if (!newUrls.Contains(oldUrl))
            {
                try
                {
                    await storage.DeleteByUrlAsync(oldUrl, ct);
                }
                catch
                {
                    // Ignore R2 deletion failure
                }
            }
        }
    ...
    public async Task<IActionResult> Delete(string id, CancellationToken ct)
    {
        ...
        await ListingDeletionService.DeleteAsync(db, listing, storage, ct);
        await db.SaveChangesAsync(ct);
        return NoContent();
    }
    ...
    [HttpPost("{id}/view")]
    public async Task<IActionResult> RecordView(string id, CancellationToken ct)
    {
        var listing = await db.Listings.FindAsync([id], ct);
        if (listing is null) return NotFound();
        ...
        {
            alreadyViewed = await db.ListingViews.AnyAsync(
                v => v.ListingId == id && (v.UserId == userId || v.IpAddress == ip), ct);
        }
        else
        {
            alreadyViewed = await db.ListingViews.AnyAsync(
                v => v.ListingId == id && v.IpAddress == ip, ct);
        }
    ...
    [HttpGet("my")]
    public async Task<ActionResult<IEnumerable<ListingDto>>> GetMyListings(CancellationToken ct)
    {
        if (!currentUser.IsAuthenticated) return Unauthorized();
    ...
    public record SubmitAppealRequest(string AppealReason);

    [HttpPost("{id}/appeal")]
    public async Task<IActionResult> AppealSuspension(
        string id,
        ...
        CancellationToken ct)
    {
        ...
        {
            return BadRequest(new { message = "Listing is not currently suspended." });
        }

        if (string.IsNullOrWhiteSpace(request.AppealReason))
        {
            return BadRequest(new { message = "Please specify an appeal reason." });
        }

        listing.AppealComment = request.AppealReason.Trim();
        await db.SaveChangesAsync(ct);

        return Ok(new { status = "AppealSubmitted" });
    }
