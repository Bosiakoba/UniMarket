    ...
    IOptions<FirebaseSettings> firebaseSettings)
{
        ...
        AppDbContext db)
    {
            ...
            try
            {
                ...
                if (user is not null)
                {
                    if (user.IsSuspended)
                    {
                        ...
                        await context.Response.WriteAsJsonAsync(new { message = "Your account has been suspended." }, context.RequestAborted);
                        return;
                    }
        ...
        {
            var devUserIdStr = devId.ToString();
            var user = await db.Users
                .AsNoTracking()
                .FirstOrDefaultAsync(u => u.Id == devUserIdStr, context.RequestAborted);
            ...
            if (user is not null)
            {
                ...
                context.Items["UserId"] = devUserIdStr;
            }
        }
