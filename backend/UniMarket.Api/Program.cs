...
builder.Services.AddSingleton<UnreadMessageEmailDispatcher>();
builder.Services.AddCors(options =>
{
    // Default REST policy — wide open for dev.
    options.AddPolicy("RestCors", policy =>
    {
    ...
    // SignalR policy — requires credentials for WebSocket transport.
...
// ---- Startup validation ----
var firebaseAtStartup = app.Services.GetRequiredService<FirebaseNotificationService>();
if (firebaseAtStartup.IsActive)
{
    firebaseAtStartup.TryInitialize();
}
else
{
    var fbOptions = app.Services.GetRequiredService<IOptions<FirebaseSettings>>().Value;
    app.Logger.LogInformation(
        "Firebase notifications are {Status}. Set Firebase__Enabled=true and provide credentials to enable.",
        fbOptions.Enabled ? "configured but not properly set up" : "disabled");
}
...
app.MapControllers();
app.MapHub<UniMarket.Api.Hubs.ChatHub>("/hubs/chat").RequireCors("SignalRCors");
app.MapGet("/health", (
